...
class FrequencyEncoder(
    ...
    def _fit(self, dataframe: DataFrame):
        ...
        for column, new_column in zip(input_columns, output_columns):
            freq = (
                dataframe.select(col(column).alias(f"g_{column}"))
                .groupby(f"g_{column}")
                .count()
        ...
        estimator = FrequencyEncoderModel(
            inputCols=input_columns, outputCols=output_columns
        )
...
class FrequencyEncoderModel(FrequencyEncoder, Transformer):
    def __init__(
        ...
        super(FrequencyEncoderModel, self).__init__(
            inputCols=inputCols, outputCols=outputCols
        )

    def _transform(self, dataframe: DataFrame):
        """
        This method transforms a DataFrame by joining it with new DataFrames created from frequency information.
        The join is performed on the input column and the first column of the new DataFrame.
        The first column of the new DataFrame is then dropped, and if the output column is not already in the DataFrame,
        a new column is added using the second column of the new DataFrame. The transformed DataFrame is returned.
        """
...
class DerivedFeatureGenerator(
    ...
    def _transform(self, dataframe: DataFrame):
        """
        This method transforms a DataFrame by casting the input columns to TimestampType and calculating the
        time difference between them. The result is divided by a value defined in the transformer object and
        assigned to a new column in the DataFrame. The transformed DataFrame is returned.
        """
        inputCols = self.getInputCols()
...
class FrequencyImputer(
    ...
    def _fit(self, dataset: DataFrame):
        ...
        estimator = FrequencyImputerModel(
...
class FrequencyImputerModel(FrequencyImputer, Transformer):
    def __init__(
        ...
        super(FrequencyImputerModel, self).__init__(
            inputCols=inputCols, outputCols=outputCols
        )
