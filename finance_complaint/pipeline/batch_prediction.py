from finance_complaint.constant import BUCKET_NAME, S3_MODEL_DIR_KEY, PYSPARK_S3_ROOT
...
class PredictionPipeline:
    def __init__(self, pipeline_config=PredictionPipelineConfig()) -> None:
        ...
        self.s3_storage: SimpleStorageService = SimpleStorageService(
            s3_bucket_name=BUCKET_NAME, region_name=pipeline_config.region_name
        )
        ...
        try:
            ...
            if len(invalid_files) > 0:
                ...
                for invalid_file in invalid_files:
                    logger.info(
                        f"Moving invalid file {invalid_file} to failed dir: {failed_dir}"
                    )
            ...
            estimator = S3FinanceEstimator(
                bucket_name=BUCKET_NAME, s3_key=S3_MODEL_DIR_KEY
            )
            for valid_file in valid_files:
                ...
                logger.info(
                    f"Writing prediction file at : [{self.pipeline_config.prediction_dir}] "
                )
                self.write_file(
