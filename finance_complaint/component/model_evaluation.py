...
from finance_complaint.data_access.model_eval_artifact import (
    ModelEvaluationArtifactData,
)
import os
...
class ModelEvaluation:
    def __init__(
        ...
        try:
            logger.info("Initiated ModelEvaluation")
    ...
    def save_ref_data(self):
        try:
            logger.info("Entered save_ref_data()")
            dataframe = self.read_data()

            dst_path = Path("artifact/ref_data/ref_data.parquet")
            dst_file = str(dst_path)

            if os.path.isfile(dst_file):
                os.remove(dst_file)
                logger.info(f"File {dst_file} removed")

            dataframe.write.parquet(dst_file)

            logger.info(f"Copied reference data to {dst_path}")
    ...
    def initiate_model_evaluation(self) -> ModelEvaluationArtifact:
        try:
            ...
            self.model_eval_artifact_data.save_eval_artifact(
